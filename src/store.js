...
const coaches = {
	...
	getters: {
		...
		isCoachCurrentUser(state, _, rootState) {
			return state.coachesData.some((coach) => coach.id === rootState.auth.currentUserId)
		},
		shouldUpdate(state) {
...
const auth = {
	...
	actions: {
		...
		async auth(context, data) {
			...
			let url = 'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyCJCxxL_72KNYKq4vuyGxgQy-YgQk_CvBU';

			if (mode === 'registration') {
				url = 'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyCJCxxL_72KNYKq4vuyGxgQy-YgQk_CvBU';
			}
